        ...
        <div key={item?.card?.info?.id} className="menuItem">
          <div className="flex justify-between pb-12 pt-6">
            <div className="categoryLeft w-[50%] mr-2 sm:w-[600px] sm:mr-0">
              ...
              <span className="rupee font-thin text-sm text-customblack-3">
                {item?.card?.info?.price / 100 ||
                  item?.card?.info?.defaultPrice / 100}
              </span>
              {item?.card?.info?.description && (
                <p className="md:w-full sm:w-9/12 mt-2 tracking-tight text-customcolor-4 text-sm">
                  {item?.card?.info?.description}
                </p>
              )}
            </div>
            <div className="categoryRight relative w-36 h-36 sm:w-[150px] sm:h-[96px]">
              ...
              <button
                className="w-20 h-9 text-sm md:w-24 md:h-9 text-[#60b246] rounded addBtn font-bold uppercase bg-white cursor-pointer absolute bottom-0 left-1/2 -translate-x-1/2"
